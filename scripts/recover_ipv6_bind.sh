...
#
# 背景: hermes-studio 升级/重装会把 dist/server/index.js 的监听默认值重置为 0.0.0.0
...
echo
echo "== 验证（需重启应用后）=="
echo "  curl -6 http://[::1]:8648/       → 应 HTTP 200（IPv6 通）"
echo "  curl    http://127.0.0.1:8648/   → 应 HTTP 200（IPv4 通）"
echo "  ss -tlnp | grep 8648             → 应显示 *:8648（双栈监听）"
